...
def river_problem() -> MOProblem:
    ...
    varsl = variable_builder(
        ["x_1", "x_2"], initial_values=[0.5, 0.5], lower_bounds=[0.3-0.01, 0.3-0.01], upper_bounds=[1.0+0.1, 1.0+0.1],
    )

    ideal = np.array([ -6.339, -2.864, -7.499, -11.626, 0 ])
    nadir = np.array([ -4.751, -2.767, -0.321, -1.920, 0.349 ])
    problem = MOProblem(variables=varsl, objectives=[f1, f2, f3, f4, f5], ideal=ideal, nadir=nadir)

    return problem

@pytest.mark.nimbus
def test_init_w_start_point(river_problem):
    starting_point = np.array([-5, -2.8, -3, -10, 0.2], dtype=float)
    method = NIMBUS(river_problem, starting_point=starting_point, scalar_method="scipy_minimize")
